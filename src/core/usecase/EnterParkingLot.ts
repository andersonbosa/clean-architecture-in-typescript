import { ParkingLotIsCloseError, ParkingLotIsFullError } from '../../errors'
...
export default class EnterParkingLot {
  ...
  async execute (code: string, plate: string, date: Date) {
    ...
    if (!parkingLot.isOpen(parkedCar.date)) {
      throw new ParkingLotIsCloseError("The parking lot is closed")
    }

    if (parkingLot.isFull()) {
      throw new ParkingLotIsFullError("The parking lot is full")
    }
